...
begin
  ...
  QTemp.SQL.Text := 'SELECT Debe - Haber As SubTotal,"' +
    EmpresaQuery.FieldByName('Nombre').AsString + '" As Empresa,* FROM "Caja"';
  QTemp.Open;

  for i := 1 to QTemp.RecordCount do
  begin
    Debe.Caption := FloatToStr(StrToFloat(Debe.Caption) +
      QTemp.FieldByName('Debe').AsFloat);
    Haber.Caption := FloatToStr(StrToFloat(Haber.Caption) +
      QTemp.FieldByName('Haber').AsFloat);
...
var
  numfact, i, OrdTrans: INTEGER;
  Fecha, FechaVence, iv: String;
  FPagoOK: Boolean;
  Detalle, MntContado, MntCheque, NroCheque, DetCheque, DiaCheque, MntTarjeta,
    NroTarjeta, DetTarjeta, MntOtros, Saldo, Pagado: String;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        QTemp.SQL.Text :=
          'Insert Into "Cheque" (Numero, Detalle, CodFactura, MntCheque, Fecha, Dias) Values'
      ...
      begin
        ...
        begin
          QTemp.SQL.Text :=
            ...
            QuotedStr('[Cheque] ' + Detalle) + ', ' + MntCheque + ', ' +
        ...
        begin
          QTemp.SQL.Text :=
            'Insert Into "Caja" (Detalle, Haber, Fecha, Hora) Values' + ' (' +
            QuotedStr('[Efectivo] ' + Detalle) + ', ' + MntContado + ', ' +
            QuotedStr(DateToStr(Date)) + ', ' +
            QuotedStr(TimeToStr(Time)) + ')';
          QTemp.ExecSQL;
        end;
      ...
      begin
        ...
        begin
          QTemp.SQL.Text :=
            'Insert Into "Caja" (Detalle, Debe, Fecha, Hora) Values' + ' (' +
            QuotedStr('[Cheque] ' + Detalle) + ', ' + MntCheque + ', ' +
            QuotedStr(DateToStr(Date)) + ', ' +
            QuotedStr(TimeToStr(Time)) + ')';
          QTemp.ExecSQL;
...
procedure TABMCajaForm.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  IF Key = VK_F4 then
    Procesar('Ingresar');
