...
uses
  ...
  ExtCtrls, OleCtrls, SHDocVw, IBX.IBCustomDataSet, IBX.IBQuery;
...
procedure TRendicionClienteForm.Quicksort(Grid: TStringGrid;
...
procedure TRendicionClienteForm.Sortgrid(Grid: TStringGrid;
...
begin
  ...
  Query.SQL.Text := 'SELECT Cobrador.Codigo,' +
    ...
    ' FROM Clientes' + ' INNER JOIN (Cobrador' + ' INNER JOIN (CtaCteItem' +
    ' INNER JOIN CtaCte' + ' ON CtaCteItem.CodCliente = CtaCte.CodCliente)' +
    ' ON Cobrador.Codigo = CtaCte.Cobrador)' +
    ' ON Clientes.CodCliente = CtaCte.CodCliente';
  FormatSettings.ShortDateFormat := '#yyyy/mm/dd#';
  Query.SQL.Add(' WHERE (CtaCte.Factura =' + Edit2.Text +
    ') AND (CtaCte.CodCliente =' + Edit5.Text +
    ') ORDER BY CtaCteItem.Vencimiento, CtaCteItem.IDCtaCteItem');
  Query.Open;
  For i := 1 to Query.RecordCount do
  begin
    if (Query.FieldByName('Vencimiento').AsDateTime) < Date then
  ...
  Query.SQL.Text := 'SELECT Cobrador.Codigo,' + ' "' + inttostr(cvencidas) +
    ...
    ' INNER JOIN CtaCte' + ' ON CtaCteItem.CodCliente = CtaCte.CodCliente)' +
    ' ON Cobrador.Codigo = CtaCte.Cobrador)' +
    ' ON Clientes.CodCliente = CtaCte.CodCliente' + ' WHERE (CtaCte.Factura =' +
    Edit2.Text + ') AND (CtaCte.CodCliente =' + Edit5.Text +
    ') ORDER BY CtaCteItem.Vencimiento, CtaCteItem.IDCtaCteItem';
  Query.Open;
...
begin
  ...
  begin
    ...
    begin
      Query.SQL.Text := 'Update "CtaCteItem" Set PAGADO = ' + StringGrid1.Cells
        [4, i] + ' Where ' + '"CtaCteItem".CODIGO = ' + StringGrid1.Cells[0, i];
      Query.ExecSQL;
    end;
  end;
  For i := 1 to StringGrid1.RowCount - 1 do
  begin
    ...
    begin
      Query.SQL.Text :=
        'Insert Into "RendidoCobrador" (VENTA, CUOTA, PAGADO)  Values (' +
...
begin
  ...
  Query.SQL.Text := 'SELECT' + '  "CtaCteItem".CODIGO,' +
    ...
    '  "Cobrador".NOMBRE AS COBRADOR ' + ' FROM' + '  "CtaCte" ' +
    ...
    ' WHERE ' + '  ("CtaCteItem".VENCE <= ' +
    QuotedStr(DateToStr(FechaDateTimePicker.Date)) + ') AND ' +
